...
class PostsFormsTests(TestCase):
    ...
    def test_edit_post(self):
        ...
        self.assertEqual(
            form_data_2['text'], Post.objects.latest('pub_date').text
        )
    ...
    def test_add_comment(self):
        ...
        (1) +1 комментарий в базе
        (2) добавленный комментарий существует
        """
        comment = PostsFormsTests.post.comments.count()
        added_comment = 1
        ...
        self.assertEqual(
            PostsFormsTests.post.comments.count(),
            (comment + added_comment)
        )

        self.assertTrue(
            PostsFormsTests.post.comments.filter(text='Текст комментария')
            .exists()
        )
